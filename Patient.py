...
class Patient(BasePatient):
    def __init__(self, first_name, last_name, email, gender, phone, date_of_birth, patient_id = None):
        super(Patient, self).__init__(first_name = first_name, last_name = last_name, email = email, patient_id = patient_id)
    ...
    def set_gender(self, gender):
        if not self.validate_gender(phone):
    ...
    def full_version(self):
        return (f"first_name={self.get_first_name()}, last_name={self.get_last_name()}, date_of_birth={self.get_date_of_birth()}, email={self.get_email()},"
