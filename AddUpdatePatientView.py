...
class AddUpdatePatientView:
    ...
    """Окно для добавления нового пациента"""
    def __init__(self, root, controller, action, patient_data=None):
        ...
        tk.Label(root, text="E-mail").grid(row=3, column=0, sticky="w", padx=10, pady=5)
        ...
        if patient_data != None:
            self.first_name_entry.insert(0, self.patient_data['first_name'])
            self.last_name_entry.insert(0, self.patient_data['last_name'])
            self.email_entry.insert(0, self.patient_data['email'])
            self.gender_entry.insert(0, self.patient_data['gender'])
            self.phone_entry.insert(0, self.patient_data['phone'])
            self.disease_entry.insert(0, self.patient_data['disease'])
    ...
    def add_update_patient(self):
        """Добавление пациента в модель через контроллер"""
        try:
